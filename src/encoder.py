...
class Encoder(nn.Module):
    def __init__(self, subnets, rnns, config):
        super().__init__()
        ...
        for index, (params, rnn) in enumerate(zip(subnets, rnns), 1):
            # index sign from 1
            setattr(self, 'stage' + str(index), make_layers(params, config))
...
#        print("after subnet size")
#        print(inputs.size())
        inputs = torch.reshape(inputs, (seq_number, batch_size, inputs.size(1),
                                        inputs.size(2), inputs.size(3)))
        outputs_stage, state_stage = rnn(inputs, None,seq_len=self.input_length)
#        print("after rnn size")
#        print(inputs.size())
