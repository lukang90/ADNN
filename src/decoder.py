    ...
    def __init__(self, subnets, rnns, config):
        ...
        for index, (params, rnn) in enumerate(zip(subnets, rnns)):
            ...
            setattr(self, 'stage' + str(self.blocks - index),
                    make_layers(params, config))

    def forward_by_stage(self, inputs, state, subnet, rnn):
#        print("input shape:")
#        if inputs != None:
#            print(inputs.size())
        inputs, state_stage = rnn(inputs, state, seq_len=self.output_length)
        seq_number, batch_size, input_channel, height, width = inputs.size()
        inputs = torch.reshape(inputs, (-1, input_channel, height, width))
#        print("after rnn shape:")
#        print(inputs.size())
        inputs = subnet(inputs)
#        print("after subnet shape:")
#        print(inputs.size())
    ...
    def forward(self, hidden_states):
        # print("decoder start")
        # print(getattr(self, 'stage3'))
        # print(getattr(self, 'rnn3'))
#        print("begin decode")
